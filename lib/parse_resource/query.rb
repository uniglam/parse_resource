class Query
  ...
  def order(attr)
    criteria[:order] = ""
    allorders = attr.split(",")
    if allorders.count > 1
      criteria[:order] = allorders.collect{|o| getOrderString(o)}.join(",")
    elsif allorders.count == 1
      criteria[:order] = getOrderString(allorders.first)            
    end
    self
  end
  
  def getOrderString(orderString)
    returnString = ""
    orders = orderString.split(" ")
    if orders.count > 1
      returnString = orders.last.downcase == "desc" ? "-#{orders.first}" : "#{orders.first}"
    else
      returnString = orders.first
    end
    return returnString
  end
